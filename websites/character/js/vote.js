...
const selectWrap = document.querySelector(".voteArea__selectWrap");
...
area.forEach(btn => {
  btn.addEventListener("click", (e) => {
    ...
    let activeChild = document.querySelectorAll(".active > li");
    let liLength = activeChild.length;

    if (liLength > 3) {
      selectWrap.classList.add("scrolled");
      let totalWidth = liLength * 150;
      linker.style.width = totalWidth + "px";
    } else {
      selectWrap.classList.remove("scrolled");
    };
...
choose.forEach(c => {
  c.addEventListener("click", (e) => {
    ...
    let chooseIdx = parent.querySelector(".selectBottom__idx").textContent;
    ...
    let choosePopHref = parent.querySelector(".popBtn a").getAttribute("href");
    console.log(choosePopHref);
    ...
    if (char_length == 0 || char_length < 3 && !checkClassName) {
      ...
      cloneElement.querySelector("input").value = chooseIdx;
      cloneElement.querySelector(".about a").href = choosePopHref;
